#入口层设计：接收命令行参数，初始化客户端--》组装依赖--》启动流程#sys.path 路径里不存在app文件夹的路径 agent-nl2sql/app/scripts/ ，app所在路径处于脚本所在路径和根目录的中间，所以出现问题
# 条件：终端所在路径为项目根路径

# 解决：在终端执行指令：python -m app.scripts.build_meta_knowledge -c conf/meta_config.yaml
