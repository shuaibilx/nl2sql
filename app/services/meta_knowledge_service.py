...
class MetaKnowledgeService:
    ...
    async def _save_tables_to_meta_db(self, meta_config: MetaConfig)-> list[ColumnInfo]:
        ...
        # 保存表信息和字段信息到元数据数据库
        async with self.meta_mysql_repository.session.begin(): # 开启一个事务，成功》commit，失败》rollback
    ...
    async def _save_column_info_to_qdrant(self, column_infos: list[ColumnInfo]):
        ...
        for i in range(0, len(embedding_texts), embedding_batch_size):
            ...
            # [改进] embedding 批量调用加重试
            batch_embeddings = await retry_async(
                self.embedding_client.aembed_documents, batch_embedding_texts,
                operation_name="Embedding-aembed_documents"
            )
